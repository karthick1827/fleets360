...
- **Polished, handed off, closed.** Apply `{workflow.doc_standards}` in order. Execute `{workflow.external_handoffs}`; surface URLs. Set both files' `status: In Review`, `updated: {date}`. Remind the user that downstream tasks remain strictly locked until the Manager reviews and marks UX deliverables as `Approved` in Markdown Studio (`markdown.html`). Log finalization via `uv run {project-root}/_acl/scripts/memlog.py append --workspace {doc_workspace} --type event --text "spines finalized"`. Share paths. Common next after Manager approval: `acl-architecture`, `acl-create-epics-and-stories`, `acl-quick-dev`. Run `{workflow.on_complete}`.
