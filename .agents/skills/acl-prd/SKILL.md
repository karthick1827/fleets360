...
7. **Close.** Set `prd.md` frontmatter `status: In Review` and `updated` to `{date}`. Remind the user that downstream tasks (`acl-ux`, `acl-architecture`, `acl-create-epics-and-stories`) remain strictly locked until the Manager reviews and marks this PRD deliverable as `Approved` in Markdown Studio (`markdown.html`). Record finalization via `uv run {project-root}/_acl/scripts/memlog.py append --workspace {doc_workspace} --type event --text "PRD finalized"`. Share artifact paths. Common next after Manager approval: `acl-ux`, `acl-architecture`, `acl-create-epics-and-stories`; invoke `acl-help` for authoritative routing.
