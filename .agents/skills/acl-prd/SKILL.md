...
2. **Input reconciliation.** Subagent per user-supplied input against `prd.md` + `addendum.md`. Each writes its extract to `{doc_workspace}/reconcile-{slug}.md` with YAML frontmatter (`title: "Project Reconcile-{Slug}"`, `status: In Review`, `type: audit`, `created: {date}`) and returns ONLY a compact summary (input name, gaps 2-5, file path). Surface gaps — especially qualitative ideas (tone, voice, feel) the FR structure silently drops. Must happen before polish.
...
7. **Close.** Set `prd.md` (and `reconcile-brief.md` if generated) frontmatter `status: In Review` and `updated` to `{date}`. Remind the user that downstream tasks (`acl-ux`, `acl-architecture`, `acl-create-epics-and-stories`) remain strictly locked until the Manager reviews and marks Phase 2 deliverables (`Project PRD` and `Project Reconcile-Brief` if generated) as `Approved` in Markdown Studio (`markdown.html`). Record finalization via `uv run {project-root}/_acl/scripts/memlog.py append --workspace {doc_workspace} --type event --text "PRD finalized"`. Share artifact paths. Common next after Manager approval: `acl-ux`, `acl-architecture`, `acl-create-epics-and-stories`; invoke `acl-help` for authoritative routing.
